...
public class AuthHomeController implements Initializable{
	...
	public void goToAllergen() throws IOException {
		AllergenController.setPreviousUI("004.1 Accueil (authentifier).fxml");
		AllergenController.setPreviousTitle("SUBPAY - Accueil (authentifié)");
		Group acteur = new Group();
		acteur.getChildren().add(
	...
	//Bouton Ajouter aux préférences
	...
	public void goToFavoriteOrder(){
		Group acteur = new Group();
		try {
			acteur.getChildren().add(FXMLLoader.load(getClass().getResource("017 Commandes préférées.fxml")));
			visual.ControllerClient.setScene(acteur, "SUBPAY - Allergènes");
		} catch (IOException e) {
			e.printStackTrace();
		}
